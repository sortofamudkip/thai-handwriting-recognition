...
class WCGAN_GP(keras.Model):
    ...
    def get_generator(self):
        ...
        x = keras.layers.Dense(8*8*64, activation="relu")(input_layer) # foundation for 8x8 image
        x = keras.layers.Reshape((8, 8, 64))(x)
        x = keras.layers.Conv2DTranspose(64, 4, strides=2, padding="same")(x) # upsample to 16x16
        x = keras.layers.LeakyReLU(alpha=0.2)(x)
        x = keras.layers.Conv2DTranspose(64, 4, strides=2, padding="same")(x) # upsample to 32x32
        x = keras.layers.LeakyReLU(alpha=0.2)(x)
        x = keras.layers.Conv2DTranspose(64, 4, strides=2, padding="same")(x) # upsample to 64x64
        x = keras.layers.LeakyReLU(alpha=0.2)(x)
        x = keras.layers.Conv2D(1, 3, padding="same", activation="sigmoid")(x) # output 64x64 image with 1 channel (grayscale)
