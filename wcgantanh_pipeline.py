...
from gans.wcgan_gp_tanh import WCGAN_GP, GenerateImageCallback, SaveModelCallback
import logging, json
...
def run_pipeline(
    ...
    # scale the pixel values to be between -1 and 1 and reverse the pixel values (black is 1, white is 0)
    train_dataset = train_dataset.map(lambda x, y: (1-(x / 127.5), y))
    validation_dataset = validation_dataset.map(lambda x, y: (1-(x / 127.5), y))
    test_dataset = test_dataset.map(lambda x, y: (1-(x / 127.5), y))
